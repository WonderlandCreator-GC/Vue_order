      ...
      <div class="overview">
        ...
        <div class="overview-right">
          <div class="score-wrapper">
            ...
            <!-- 评分星组件 -->
            <star :score="4.6" :size="shopInfo.serviceScore" />
            <span class="score">{{ shopInfo.serviceScore }}</span>
          </div>
          <div class="score-wrapper">
            <span class="title">商品评分</span>
            <!-- 评分星组件 -->
            <star :score="4.7" :size="shopInfo.foodScore" />
          ...
          <li class="rating-item" v-for="(item, index) in filterRatings" :key="index">
            ...
            <div class="content">
              ...
              <div class="recommend">
                ...
                <span class="item" v-for="(recommend, index) in item.recommends" :key="index">南瓜粥</span>
              </div>
...
export default {
  data() {
    return {
      ...
      onlyShowText: true // 是否只显示带有文本的评价
    }
  },
  mounted() {
    // 获取商家评价信息
    // 传入一个回调函数，以实现在获取数据后创建滚动实例对象
    this.$store.dispatch('getShopRatings', () => {
      // 获取数据且页面刷新后调用$nextTick中的回调函数
      this.$nextTick(() => {
  ...
  computed: {
    ...
    // 按条件过滤评价数组
    filterRatings() {
      // 按条件过滤数组:首先要找影响筛选的所有条件
      const { shopRatings, selectType, onlyShowText } = this
      // 按条件过滤产生一个新数组
      return shopRatings.filter(rating => {
        /*
        条件1：
          selectType可能有的值：0/1/2
          rateType可能有的值:0/1
          最终条件:selectType === 2 || selectType === rating.rateType

        条件2：
          onlyShowText：ture/false
          text: 有文本或没文本
